 ...
 *
 * xy_where(X, Y, Where), put(@cb, queen, black, Where).
 *
 *
 */
...
% nreinas(N, Queens) :- length(Queens, N), vector(Sol, N), genera(Queens, Sol). genera([], []).
% genera([X|Xs], [c(_, X)|Ys]) :- genera(Xs, Ys).
...
/*------------------------------------------------------------
test_square(queen, black, white, Image).
...
/*----------------------------------------------------------------------
 make_chess_board(@cb, N), new(W, window('Chess Board', size(N*32,
 N*32))), send(W, display, @cb), send(W, open).

get(@cb, member, e2, Bitmap), get(Bitmap, square_colour, SquareColour),
get(Bitmap, piece, Piece).

-----------------------------------------------------------------------*/

put(Board, Piece, Colour, Where) :-
